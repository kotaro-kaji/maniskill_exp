...
def print_matrix(name, R):
    print(name)
    for row in R:
        print("  " + " ".join(f"{v: .7f}" for v in row))
...
for step in range(500):
    ...
    for key, agent, target_R in zip(keys, agents, targets):
        ...
        command[3:6] = rot_action_sign * np.clip(rotvec / max_rot_step, -1.0, 1.0)
        action[key] = command
    env.step(action)

print_matrix("target_left", target_left)
print_matrix("final_left", tcp_rotation(agents[0], env.unwrapped.device))
print_matrix("target_right", target_right)
print_matrix("final_right", tcp_rotation(agents[1], env.unwrapped.device))

for name, agent, target_R in zip(["left", "right"], agents, targets):
    final_R = tcp_rotation(agent, env.unwrapped.device)
    angle_error = np.linalg.norm(matrix_to_rotvec(target_R @ final_R.T))
    print(f"{name}_angle_error_deg {math.degrees(angle_error):.6f}")
