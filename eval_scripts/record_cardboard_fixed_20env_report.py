...
def render_html(
    *,
    ...
    disable_panel_obs_step_noise: bool,
):
    title = f"{checkpoint.name} {robot_uid} Randomized v2 Samples"
    noise_text = "disabled" if disable_panel_obs_step_noise else "enabled"
...
<body>
  <main>
    ...
    <p class="meta">Metrics: 160 env eval, base seed 1, {html.escape(ENV_ID)}, robot_init_noise_scale 0.05, panel obs step noise {noise_text}. Videos: one-env replay with seed = 1 + env_idx.</p>
    <div class="grid">
{''.join(sections)}
    </div>
  </main>
...
def main():
    ...
    suffix = "_no_panel_step_noise" if args.disable_panel_obs_step_noise else ""
    run_name = f"{safe_name(checkpoint.stem)}_{safe_name(args.robot_uid)}{suffix}"
    ...
    if args.disable_panel_obs_step_noise:
        common_args.append("--disable-panel-obs-step-noise")
    metrics_cmd = common_args + [
        "--output-dir",
        str(video_dir / "_metrics_no_video"),
        "--skip-video",
    ]
    ...
    for env_idx in ENV_INDICES:
        tmp_dir = video_dir / f"_env_{env_idx}"
        if tmp_dir.exists():
            shutil.rmtree(tmp_dir)
        dst_video = video_dir / f"env_{env_idx}.mp4"
        if dst_video.exists():
            continue
        video_cmd = [
            sys.executable,
        ...
        if args.disable_panel_obs_step_noise:
            video_cmd.append("--disable-panel-obs-step-noise")
        ...
        print(video_result.stdout, end="")
        src_video = tmp_dir / "env_0.mp4"
        assert src_video.exists(), src_video
    ...
    render_html(
        ...
        html_path=html_path,
        metrics=metrics,
        disable_panel_obs_step_noise=args.disable_panel_obs_step_noise,
    )
