...
# Post-curriculum kinematics-URDF small-delta fine-tuning:
...
#
# Usage:
#   bash scripts/train_cardboard_kinematics_delta060_to_delta020.sh <randomized_v2_delta060_best_ckpt>
#   PYTHON_BIN=.venv/bin/python bash scripts/train_cardboard_kinematics_delta060_to_delta020.sh <ckpt>
...
INPUT_CKPT="$1"
SEED="${SEED:-1}"
RUN_PREFIX="${RUN_PREFIX:-cardboard_kinematics_delta060_to_delta020}"
...
run_delta_stage "delta03_lr1e4_only" \
  my_xarm7_kinematics_delta03 "${INPUT_CKPT}" "${TOTAL_TIMESTEPS_DELTA03}" 1024 32
delta03_ckpt="$(best_ckpt "runs/${RUN_PREFIX}_seed${SEED}_delta03_lr1e4_only" 1024)"

run_delta_stage "delta02_lr1e4_only" \
  my_xarm7_kinematics_delta02 "${delta03_ckpt}" "${TOTAL_TIMESTEPS_DELTA02}" 512 16
