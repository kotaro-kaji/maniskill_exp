...
# Reproducible small-delta fine-tuning chain:
...
#   stage 3: continue from the stage-2 best checkpoint with my_xarm7_delta015
#
# The delta=0.015 stage follows the successful low-LR anchored run:
#   checkpoint: delta=0.02 best
#   anchor checkpoint: same delta=0.02 best
#   lr=3e-5, anchor=0.05, clip=0.05, target_kl=0.03
#
# Usage:
#   bash scripts/train_cardboard_delta060_to_delta015.sh <randomized_v2_delta060_best_ckpt>
#   PYTHON_BIN=.venv/bin/python bash scripts/train_cardboard_delta060_to_delta015.sh <ckpt>
...
TOTAL_TIMESTEPS_DELTA015="${TOTAL_TIMESTEPS_DELTA015:-15000000}"
...
run_delta015_stage() {
  ...
  echo
  echo "### delta015_lowlr_anchor005: my_xarm7_delta015"
  printf '%q ' "${cmd[@]}"
  echo
  "${cmd[@]}"
}
...
run_delta_stage "delta03_lr1e4_only" \
  my_xarm7_delta03 "${INPUT_CKPT}" "${LEARNING_RATE_DELTA03}" \
...
run_delta_stage "delta02_lr1e4_only" \
  my_xarm7_delta02 "${delta03_ckpt}" "${LEARNING_RATE_DELTA02}" \
