...
# Continue the kinematics-URDF cardboard curriculum from a prepared stage1
# delta=0.06 checkpoint:
#   stage2 randomized-v1 -> stage3 randomized-v2 -> delta=0.03 -> delta=0.02

if [[ $# -ne 1 ]]; then
  echo "Usage: $0 <stage1_delta060_ckpt>" >&2
  exit 2
fi

STAGE1_CKPT="$1"
if [[ ! -f "${STAGE1_CKPT}" ]]; then
  echo "stage1 checkpoint does not exist: ${STAGE1_CKPT}" >&2
  exit 2
fi
...
DELTA020_RUN_PREFIX="${DELTA020_RUN_PREFIX:-cardboard_kinematics_delta060_to_delta020}"
...
echo
echo "### kinematics stage2 randomized-v1 and stage3 randomized-v2 from stage1 checkpoint"
RUN_PREFIX="${THREE_STAGE_RUN_PREFIX}" \
PYTHON_BIN="${PYTHON_BIN}" \
NUM_ENVS="${THREE_STAGE_NUM_ENVS}" \
SEED="${SEED}" \
CHECKPOINT_STAGE1="${STAGE1_CKPT}" \
...
echo
echo "### continue kinematics delta=0.06 -> delta=0.03 -> delta=0.02"
RUN_PREFIX="${DELTA020_RUN_PREFIX}" \
PYTHON_BIN="${PYTHON_BIN}" \
SEED="${SEED}" \
bash scripts/train_cardboard_kinematics_delta060_to_delta020.sh "${stage3_ckpt}"

echo
echo "### kinematics resumed curriculum input checkpoint summary"
