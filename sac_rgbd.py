...
class PlainConv(nn.Module):
    def __init__(self,
        ...
        if (image_size[0], image_size[1]) in [(128, 128), (256, 192)]:
            first_pool = 4
        else:
            first_pool = 2
        ...
        with torch.no_grad():
            ...
            if self.pool is not None:
                x = self.pool(x)
            n_flatten = x.flatten(1).shape[1]
        self.fc = make_mlp(n_flatten, [out_dim], last_act=last_act)
...
class Actor(nn.Module):
    def __init__(self, envs, sample_obs):
        ...
        assert image_size == (192, 256), f"Only 256x192 supported, got {image_size}"
...
if __name__ == "__main__":
    ...
    if eval_video_envs is not None:
        eval_video_envs = FlattenRGBDObservationWrapper(eval_video_envs, rgb=True, depth=False, state=args.include_state)
    ...
    if eval_video_envs is not None:
        eval_video_obs, _ = eval_video_envs.reset(seed=args.seed)
    ...
    while global_step < args.total_timesteps:
        if args.eval_freq > 0 and (global_step - args.training_freq) // args.eval_freq < global_step // args.eval_freq:
            ...
            if eval_video_envs is not None:
                ...
                for _ in range(args.num_eval_steps):
                    with torch.no_grad():
                        eval_video_actions = actor.get_eval_action(eval_video_obs)
                        eval_video_obs, _, _, _, _ = eval_video_envs.step(eval_video_actions)
            eval_metrics_mean = {}
        ...
        if (global_step - args.training_freq) // args.log_freq < global_step // args.log_freq:
            ...
            if torch.cuda.is_available():
                ...
                mb = 1024 * 1024
                logger.add_scalar("gpu/memory_allocated_mb", torch.cuda.memory_allocated(device_idx) / mb, global_step)
                logger.add_scalar("gpu/memory_reserved_mb", torch.cuda.memory_reserved(device_idx) / mb, global_step)
                logger.add_scalar("gpu/max_memory_allocated_mb", torch.cuda.max_memory_allocated(device_idx) / mb, global_step)
                logger.add_scalar("gpu/max_memory_reserved_mb", torch.cuda.max_memory_reserved(device_idx) / mb, global_step)
